...
~\\ $L(x,\lambda) = (-\lambda n) + Ln(\lambda{\sum_{i=1}^N{x_{i}}}) - (Ln\sum_{i=1}^N{x_{i}})$
~\\ $L(x,\lambda) = -(\lambda n) + {\sum_{i=1}^N{x_{i}}}Ln(\lambda) - (Ln\sum_{i=1}^N{x_{i}})$

~\\ $\frac{dL(x;\lambda)}{d\lambda} = \frac{d}{d\lambda}(-\lambda n + {\sum_{i=1}^N{x_{i}}}Ln(\lambda) - (Ln\sum_{i=1}^N{x_{i}}))$
...
~\\ $L(\theta; y_{1},y_{2},...,y_{n})=\prod \limits_{i=0}^{n}(e^{-(y-\theta)})=e^{-\sum \limits_{i=1}^n {(y_{i}-\theta)}}$
~\\
